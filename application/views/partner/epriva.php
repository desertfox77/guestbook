      ...
      <div id="content">
        ...
        <div class="container-fluid">
          ...
          <div class="d-sm-flex align-items-center justify-content-between mb-4">
            <h1 class="h3 mb-0 text-gray-800">Edit Private Class</h1>
            </div>
            <?php
            $id = $_GET['id'];
	$leo= $this->db->query("SELECT * FROM private WHERE id='$id'"); 
	foreach($leo->result_array() as $d){
		?>
            <div class="row">
          <div class="col-lg-6">
              <form action="<?= base_url('Partner/epriva')?>" method="post">
              <?= $this->session->flashdata('message'); ?>
              <div class="form-group">
              <input type="hidden" name="id" value="<?php echo $d['id']; ?>">
    <label for="class_name" >Class Name</label>
      <input type="label" class="form-control" id="class_name" name="class_name" value="<?= $d['name']; ?>" required>
...
<br>
    ...
    <label for="class_price" >Class Price</label>
      <input type="number" class="form-control" id="class_price" name="class_price" value="<?= $d['price']; ?>" required>
      <?= form_error('class_price', '<small class="text-danger pl-3">', '</small>'); ?>
    </div>

    
    

    <select name="day" id="wgtmsr" style="width: 500px;" required>
